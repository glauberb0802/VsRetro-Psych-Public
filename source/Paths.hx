...
class Paths
{
	...
	[
		'assets/music/Menu_${Music.defaultMusic}.$SOUND_EXT',
		'assets/shared/images/characters/minus/minus_sakuroma_assets_full/spritemap.png',
	...
	public static function clearStoredMemory(?cleanUnused:Bool = false) {
		...
		if (PlayState.isStoryMode && PlayState.firstTry && !PlayState.seenCutscene)
			{
				currentTrackedSounds = [];
				var gottenPath:String = 'assets/music/${MainMenuState.songName}.$SOUND_EXT';
		...
		for (key in OpenFlAssets.cache.getFontCache().keys())
	...
	inline static public function txtRaw(key:String, ?library:String)
	{
	...
	inline static public function json(key:String, ?library:String, suffix:String = '')
	{
		return getPath('data/$key$suffix.json', TEXT, library);
	}
	...
	{
		var songKey:String = '${song.toLowerCase().replace(' ', '-')}/Voices$suffix';
	...
	{
		var songKey:String = '${song.toLowerCase().replace(' ', '-')}/Inst$suffix';
	...
	public static function doesFileExist(file:String, type:lime.utils.AssetType = TEXT) {
		#if sys
		return Util.exists(file);
		#else
		return Assets.exists(file, type);
		#end
	}

	public static function imageExists(key:String, ?library:String) {
		#if MODS_ALLOWED
	...
	public static function returnGraphic(key:String, ?library:String, text:Bool = false) {
		...
		//the texture compression code was originally done by Yoshubs from this link: https://github.com/Yoshubs/Forever-Engine-Legacy/blob/master/source/Paths.hx
		//this is an experimental optimization that reduces at least 300-400 MB of ram on characters, however, for odd reasons it builds ram so it is disabled for the time being.
		//if the repo owner or shubs wishes to remove these 6 lines of code, it will be removed per request.
		var path = getPath('images/$key.png', IMAGE, library);
		if (OpenFlAssets.exists(path, IMAGE)) {
			if(!currentTrackedAssets.exists(path)) {
				...
				if(text && ClientPrefs.textureCompression)
				{
					var newBitmap:BitmapData = OpenFlAssets.getBitmapData(path);
					var texture = FlxG.stage.context3D.createTexture(newBitmap.width, newBitmap.height, BGRA, true, 0);
	...
	public static function getSoundPath(key:String, ?library:String) {
		//#if MODS_ALLOWED
		...
		var gottenPath:String = getPath('sounds/$key.$SOUND_EXT', SOUND, library);	
	...
	public static function returnSound(path:String, key:String, ?library:String) {
		...
		if (currentTrackedSounds.exists(gottenPath) && currentTrackedSounds.get(gottenPath) != null)
