...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Confetti Burst', "Value 1: 1 is regular burst, 2 is big burst"],
	...
	var curRenderedSustains:FlxTypedGroup<FlxSpriteExtra>;
	var curRenderedNotes:FlxTypedGroup<Note>;
	var curRenderedNoteType:FlxTypedGroup<FlxFixedText>;

	var nextRenderedSustains:FlxTypedGroup<FlxSpriteExtra>;
	...
	public static var quants:Array<Float> = [
		4,// quarter
		2, // half
		4/3,
		1,
	...
	static var vortexQuantColor:Array<FlxColor> = [
		0xFFFF0000,
		0xFF0033FF,
		0xFFCC00FF,
		0xFFFFCC00,
	...
	override function create()
	{
		...
		var song = StringTools.replace(_song.song, '-', ' ');
		#if !final
		//song = "song";
		#end
		DiscordClient.changePresence("Chart Editor", song);
		#end
		...
		curRenderedSustains = new FlxTypedGroup<FlxSpriteExtra>();
		curRenderedNotes = new FlxTypedGroup<Note>();
		curRenderedNoteType = new FlxTypedGroup<FlxFixedText>();

		nextRenderedSustains = new FlxTypedGroup<FlxSpriteExtra>();
		...
		bpmTxt = new FlxFixedText(1000, 50, Std.int(FlxG.width/3), "", 16);
		...
		quant = new AttachedSpriteColor('quant');
		...
		var tipTextArray:Array<String> = text.replace('\r', '').split('\n');
		for (i in 0...tipTextArray.length) {
			var st = tipTextArray[i].trim();
			if(st == "") continue;
			var tipText = new FlxFixedText(UI_box.x, UI_box.y + UI_box.height + 8, 0, st, 16);
			tipText.y += i * 14;
		...
		zoomTxt = new FlxFixedText(10, 10+20, 0, "Zoom: 1x", 16);
	...
	function addSongUI():Void
	{
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			#if sys
			if (#if MODS_ALLOWED Util.exists(Paths.modsJson(songName + '/events')) || #end Util.exists(file))
			#else
		...
		var steppervocvol:FlxUINumericStepper = new FlxUINumericStepper(90, stepperBPM.y + 35, 0.1, 1, 0.1, 1, 1);
		_song.vocalVol = 1;//default in FlxUINumericStepper not working for some reason??
		steppervocvol.value = _song.vocalVol;
		steppervocvol.name = 'song_vocvol';
		blockPressWhileTypingOnStepper.push(steppervocvol);

		var stepperinstvol:FlxUINumericStepper = new FlxUINumericStepper(90, 70, 0.1, 1, 0.1, 1, 1);
		_song.instVol = 1;
		stepperinstvol.value = _song.instVol;
		stepperinstvol.name = 'song_instvol';
		blockPressWhileTypingOnStepper.push(stepperinstvol);
		...
		var skin = PlayState.SONG.arrowSkinbf;
		if(skin == null) skin = '';
		var skin2 = PlayState.SONG.arrowSkindad;
		if(skin2 == null) skin2 = '';
		noteSkinInputTextbf = new FlxUIInputText(player2DropDown.x, player2DropDown.y + 50, 125, skin, 8);
		blockPressWhileTypingOn.push(noteSkinInputTextbf);

		noteSkinInputTextdad = new FlxUIInputText(player2DropDown.x + 150, player2DropDown.y + 50, 125, skin2, 8);
		blockPressWhileTypingOn.push(noteSkinInputTextdad);

		noteSplashesInputText = new FlxUIInputText(noteSkinInputTextbf.x, noteSkinInputTextbf.y + 35, 125, _song.splashSkin, 8);
		...
		tab_group_song.add(steppervocvol);
		tab_group_song.add(stepperinstvol);
		tab_group_song.add(reloadNotesButton);
		tab_group_song.add(noteSkinInputTextbf);
		tab_group_song.add(noteSkinInputTextdad);
		tab_group_song.add(noteSplashesInputText);
		tab_group_song.add(new FlxText(stepperBPM.x, stepperBPM.y - 15, 0, 'Song BPM:'));
		tab_group_song.add(new FlxText(stepperinstvol.x, stepperinstvol.y - 15, 0, 'Inst Vol:'));
		tab_group_song.add(new FlxText(steppervocvol.x, steppervocvol.y - 15, 0, 'Vocals Vol:'));
		tab_group_song.add(new FlxText(stepperSpeed.x, stepperSpeed.y - 15, 0, 'Song Speed:'));
		tab_group_song.add(new FlxText(player2DropDown.x, player2DropDown.y - 15, 0, 'Opponent:'));
		tab_group_song.add(new FlxText(gfVersionDropDown.x, gfVersionDropDown.y - 15, 0, 'Girlfriend:'));
		tab_group_song.add(new FlxText(player1DropDown.x, player1DropDown.y - 15, 0, 'Boyfriend:'));
		tab_group_song.add(new FlxText(stageDropDown.x, stageDropDown.y - 15, 0, 'Stage:'));
		tab_group_song.add(new FlxText(noteSkinInputTextbf.x, noteSkinInputTextbf.y - 15, 0, 'Bf Note Texture:'));
		tab_group_song.add(new FlxText(noteSkinInputTextdad.x, noteSkinInputTextdad.y - 15, 0, 'Dad Note Texture:'));
				...
				for (file in Util.readDirectory(directory)) {
					...
					if (!Util.isDirectory(path) && file != 'readme.txt' && file.endsWith('.txt')) {
						...
						if(!eventPushedMap.exists(fileToCheck)) {
							...
							eventStuff.push([fileToCheck, Util.getContent(path)]);
						}
					}
				}
			}
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'song_vocvol')
			{
				_song.vocalVol = nums.value;
			}
			else if (wname == 'song_instvol')
			{
				_song.instVol = nums.value;
			}
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if(vortex) {
				...
				var controlArray:Array<Bool> = [justPressed.ONE, justPressed.TWO, justPressed.THREE, justPressed.FOUR, justPressed.FIVE, justPressed.SIX, justPressed.SEVEN, justPressed.EIGHT];
				...
				if(oldQuant != curQuant) {
					trace("QuantI = " + curQuant + " | Quant = " + quants[curQuant]);
					reloadGridLayer();
				}

				var color = FlxColor.fromRGB(125, 125, 125);
				if(curQuant < vortexQuantColor.length) {
					color = vortexQuantColor[curQuant];
				}
				quant.colorSwap.red = color;
				color.brightness *= 0.5;
				quant.colorSwap.green = color;
				quant.colorSwap.blue = 0xFFffffff;
				...
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN)
				{
					...
					var secStartTime = sectionStartTime();
					for (i in 0...cuquant) {
						datimess.push(secStartTime + daTime * i);
					}
					...
					if (FlxG.keys.pressed.UP)
					{
						...
						var time = Math.ceil(FlxG.sound.music.time);
						while (i > -1) {
							if (time >= Math.ceil(datimess[i])) {
					...
					{
						var time = Math.floor(FlxG.sound.music.time);
						for (i in datimess) {
							if (time <= Math.floor(i)) {
					...
					if(songTimeTween != null) {
						songTimeTween.cancel();
						songTimeTween.destroy();
					}

					songTimeTween = FlxTween.tween(FlxG.sound.music, {time: feces}, 0.1, {ease:FlxEase.circOut, onComplete: function(twn:FlxTween) {
						songTimeTween = null;
					}});
					...
					if (curSelectedNote != null)
					{
						...
						var controlArray:Array<Bool> = [pressed.ONE, pressed.TWO, pressed.THREE, pressed.FOUR, pressed.FIVE, pressed.SIX, pressed.SEVEN, pressed.EIGHT];

						if(controlArray.contains(true))
						{
							var daStrumTime = curSelectedNote[0];
							var daTime = feces - daStrumTime; // (feces - secStart) - (daStrumTime - secStart); //idk math find out why it doesn't work on any other section other than 0
		...
		var curSecMustHit = _song.notes[curSection].mustHitSection;
		curRenderedNotes.forEachAlive(function(note:Note) {
			note.alpha = 1;
			if(curSelectedNote != null) {
				var noteDataToCheck:Int = note.noteData;
				if(noteDataToCheck > -1 && note.mustPress != curSecMustHit) noteDataToCheck += 4;
				...
				//note.quantColor = noteDataToCheck;
				//note.quantEnabled = !note.quantEnabled;
				//note.quantEnabled = !note.quantEnabled;
			}
			...
			if(note.strumTime <= Conductor.songPosition) {
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data]) {
						...
						data = note.noteData;
						if(note.mustPress != curSecMustHit)
						{
							data += 4;
						}
					}
	...
	function loadAudioBuffer() {
		...
		audioBuffers[0] = null;
		#if MODS_ALLOWED
		if(Util.exists(Paths.modFolders('songs/' + currentSongName + '/Inst.ogg'))) {
		...
		audioBuffers[1] = null;
		#if MODS_ALLOWED
		if(Util.exists(Paths.modFolders('songs/' + currentSongName + '/Voices.ogg'))) {
	...
	static var GRID_MH = GRID_SIZE * 32;
	static var GRID_H = GRID_MH * 3;
	static var GRID_Y_OFF = GRID_MH;

	var gridBlackLineEvent:FlxSpriteExtra;
	var gridBlackLine:FlxSpriteExtra;
	...
	function reloadGridLayer() {
		...
		var lighter = 0xffe7e6e6; //0xff1e1e1e;
		var darker = 0xffd9d5d5; //0xff121212;
		if(oldZoom != zoomList[curZoom]) {
			if(gridBG != null) gridBG.destroy();
			gridBG = FlxGridOverlay.create(1, 1, 1 * 9, Std.int(1 * 32 * zoomList[curZoom]), true, lighter, darker);
			gridBG.scale.set(GRID_SIZE, GRID_SIZE);
			gridBG.updateHitbox();
			oldZoom = zoomList[curZoom];
		}
		...
		var gridBlack = new FlxSpriteExtra(0, gridBG.height / 2).makeSolid(Std.int(GRID_SIZE * 9), Std.int(gridBG.height / 2), FlxColor.BLACK);
		gridBlack.alpha = 0.4;
		gridLayer.add(gridBlack);

		var gridBlackLine = new FlxSpriteExtra(gridBG.x + gridBG.width - (GRID_SIZE * 4)).makeSolid(2, Std.int(gridBG.height), FlxColor.BLACK);
		gridLayer.add(gridBlackLine);

		if(vortex) {
			var quantColor = vortexQuantColor[curQuant];
			var quant = quants[curQuant];
			var total = Math.round(32/quant/2);

			var offset = 0;//(GRID_SIZE * 16);
			//var offset = (GRID_SIZE * curSection * 16);
			//if(curSection != 0) {
			//	offset -= (GRID_SIZE * 16);
			//}

			for (i in 1...total) {
				var beatsep1 = new FlxSpriteExtra(gridBG.x).makeSolid(Std.int(gridBG.width), 1);
				beatsep1.color = quantColor;
				beatsep1.y = offset + (GRID_SIZE * (quant * zoomList[curZoom]))*i;
				gridLayer.add(beatsep1);
			}
		}

		gridBlackLine = new FlxSpriteExtra(gridBG.x + GRID_SIZE).makeSolid(2, Std.int(gridBG.height), FlxColor.BLACK);
		gridLayer.add(gridBlackLine);
		updateGrid();
	}
	...
	function updateHeads():Void
	{
		...
		if (_song.notes[curSection].mustHitSection)
		{
			...
			else leftIcon.changeIcon(healthIconP1);
			rightIcon.changeIcon(healthIconP2);
		}
		else
		{
			if (_song.notes[curSection].gfSection) leftIcon.changeIcon('gf');
			else leftIcon.changeIcon(healthIconP2);
	...
	{
		clearNow(curRenderedNotes);
		clearNow(nextRenderedNotes);
		clearNow(curRenderedSustains);
		clearNow(nextRenderedSustains);
		clearNow(curRenderedNoteType);
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		var note:Note = new Note(daStrumTime, daNoteData % 4, false, null, null, true);
		if(daSus != null) { //Common note
			...
			note.playNoteAnim();
			note.angle = note.offsetAngle + note.offsetAngleQuant;
		} else { //Event note
		...
		note.setGraphicSize(GRID_SIZE, GRID_SIZE);
		note.updateHitbox();
		note.x = Math.floor(daNoteData * GRID_SIZE) + GRID_SIZE;
	...
	function setupSusNote(note:Note):FlxSpriteExtra {
		...
		var spr:FlxSpriteExtra = new FlxSpriteExtra(note.x + (GRID_SIZE * 0.5) - 4, note.y + GRID_SIZE / 2).makeSolid(8, height);
		spr.active = false;
		return spr;
	}
	...
	function loadJson(song:String):Void
	{
		...
		if (Difficulty.difficulties[PlayState.storyDifficulty] != "Normal") {
	...
	private function saveLevel()
	{
		...
		var tempSectionHolder = curSection;
		curSection = _song.notes.length - 1;
		while (sectionStartTime() > FlxG.sound.music.length)
		{
			_song.notes.remove(_song.notes[curSection]);
			curSection--;
		}
		curSection = tempSectionHolder;
		...
		if ((data != null) && (data.length > 0))
		{
			...
			if (Difficulty.difficulties[PlayState.storyDifficulty] != "Normal") {
				difficultyEnding = "-"+Difficulty.difficulties[PlayState.storyDifficulty].toLowerCase();
			}
			_file.save(data.trim(), Paths.formatToSongPath(_song.song) + difficultyEnding + ".json");
		}
	}
	...
	private function saveEvents()
	{
		...
		var eventsSong:SwagSong = {
			...
			speed: _song.speed,
			arrowSkinbf: _song.arrowSkinbf,
			arrowSkindad: _song.arrowSkindad,
			...
			stage: _song.stage,
			validScore: false,
			instVol: _song.instVol,
			vocalVol : _song.vocalVol
		};
		var json = {
			"song": eventsSong
		}
