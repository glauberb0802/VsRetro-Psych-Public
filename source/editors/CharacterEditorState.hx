...
class CharacterEditorState extends MusicBeatState
{
	...
	function addSettingsUI() {
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			for (character in characters)
			{
				...
				{
					if(anim.scale != null)
						character.addScale(anim.anim, anim.scale);
				}
				...
				character.forceDance = parsedJson.forceDance;
	...
	function addAnimationsUI() {
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 30, "Add/Update", function() {
			...
			if(!char.animScales.exists(newAnim.anim)) {
				char.addScale(newAnim.anim, 1);
			}
		...
		var removeButton:FlxButton = new FlxButton(180, animationIndicesInputText.y + 30, "Remove", function() {
			for (anim in char.animationsArray) {
				if(animationInputText.text == anim.anim) {
					...
					if(char.animScales.exists(anim.anim)) {
						char.animScales.remove(anim.anim);
					}
					char.animationsArray.remove(anim);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			if (sender == scaleStepper)
			{
				...
				char.characterScale.set(char.jsonScale, char.jsonScale);
	...
	function genBoyOffsets():Void
	{
		...
		for (anim in char.animationsArray)
		{
			...
			var scale = anim.scale != null ? anim.scale : 1;
			var text:FlxText = new FlxText(10, 20 + (18 * daLoop), 0, animName + ": " + offsets + ' (Scale = ${scale})', 15);
	...
	function reloadGhost() {
		...
		for (anim in char.animationsArray) {
			...
			if(anim.scale != null) {
				ghostChar.addScale(anim.anim, anim.scale);
			}
		}
		...
		ghostChar.characterScale.set(ghostChar.jsonScale, ghostChar.jsonScale);
	...
	override function update(elapsed:Float)
	{
		...
		if(!charDropDown.dropPanel.visible) {
			...
			if(char.animationsArray.length > 0) {
				...
				var controlArray:Array<Bool> = [FlxG.keys.justPressed.N, FlxG.keys.justPressed.M];
				...
				for (i in 0...controlArray.length) {
					if(controlArray[i]) {
						...
						var negaMult:Int = (i * 2) - 1; //-1 or 1
						var scale:Float = 1;

						if(char.animScales.exists(animName)) scale = char.animScales.get(animName);
						scale += 0.005 * multiplier * negaMult;

						char.animationsArray[curAnim].scale = scale;
						char.addScale(animName, scale);
						ghostChar.addScale(animName, scale);

						char.playAnim(animName, false);
						if(ghostChar.animation.curAnim != null && char.animation.curAnim != null && char.animation.curAnim.name == ghostChar.animation.curAnim.name) {
							ghostChar.playAnim(char.animation.curAnim.name, false);
						}
						genBoyOffsets();
	...
	function saveCharacter() {
		...
		if(char.extraImages.length > 0) {
			Reflect.setField(json, "extraImages", char.extraImages);
		}
