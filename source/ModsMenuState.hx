...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		removeButton = new FlxButton(startX, 620, "Delete Selected Mod", function()
		{
			...
			if(Util.exists(path) && Util.isDirectory(path))
			{
				trace('Trying to delete directory ' + path);
				try
				{
					Util.deleteFile(path); //FUCK YOU HAXE WHY DONT YOU WORK WAAAAAAAAAAAAH
		...
		while (i < modsList.length)
		{
			...
			if(!Util.exists(Paths.mods(values[0])))
			{
